...
export default class productionDB {
    ...
    //Inserta el nuevo dato maestro de la compañía
    static registerProductionProductsDB(newInfo, user, dayId) {
        ...
        keys.map(key => {
            ...
            FROM product
            WHERE nameProduct LIKE '${key}';

            UPDATE product SET stock= stock+${newInfo[key]} WHERE nameProduct LIKE '${key}';
        ...
        //SELECT idProduct FROM product WHERE nameProduct LIKE 'Blush';
    ...
    //Trae los números de telefonos de las personas para el reporte
    static getCellphonesNumber() {
        let query = ''
        try {
            query = `SELECT cellphone FROM employee WHERE idOcupation in (1,2,3)`
    ...
    static getStock() {
        let query = ''
        try {
            query = `SELECT nameProduct, stock FROM product`
    ...
    static newMasterDataDB(type, newInfo, form, user) {
        ...
        try {
            ...
            if (form == "product") {
                query = `INSERT INTO product (quantityStems, nameProduct, idVarietyPlant, active, createdAt, createdBy) 
                VALUES ( '${newInfo.quantityStems}', '${newInfo.nameProduct}', '${newInfo.idVarietyPlant.value}', '1', CURRENT_TIMESTAMP, '${user}');`
            }
