import { connectionAuth, AUPPConnection, connectionMIS, DataBotSSConnection } from "../connection";

export default class authDB {

    static signInConsult(email, password) {
        ...
        try {
            ...
            //query = `select * from orchestrator`;
            console.log(query);

            return new Promise((resolve, reject) => {
                try {
                    connectionAuth.query(query, (error, results) => {
                        if (error) {
                            reject(error)
                        } else {
                            resolve(results)
